...
interface Props extends BoxProps {//// props get extended from BoxProps, but there is no describtion about BoxProps
...
const WalletPage: React.FC<Props> = (props: Props) => {
  ...
  const prices = usePrices();
	...
	const getPriority = (blockchain: any): number => {////blockchain type can be string to make it more safety and more meaning
	  switch (blockchain) {
	    case 'Osmosis':
	      return 100
	    case 'Ethereum':
  ...
  const sortedBalances = useMemo(() => {
    return balances.filter((balance: WalletBalance) => {
		  ...
		  if (lhsPriority > -99) { //// some variables in this code haven't get declared, lhsPriority and BoxProps didn't get declared, and also get mention only 1 time in the code
        ...
        ////in that case i think this is a variable name mistake and balancePriority should replace lhsPriority as it never get used
		  ...
		  if (leftPriority > rightPriority) {
		    return -1;
		  } else if (rightPriority > leftPriority) {////more nested if statement that can reduce to 1 or less
  ...
  const formattedBalances = sortedBalances.map((balance: WalletBalance) => {//formattedBalances didn't get use yet
    return {
      ...balance,
